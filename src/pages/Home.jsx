...
export default function Home() {
  ...
  const {
    user, signOut,
    syncRecipe, syncRating, syncNote, syncFavourite,
    syncProfile, syncMealPlan, syncDailyLog, syncWeeklyTrend,
  } = useSync()
  const { search: searchUSDA, loading: usdaLoading } = useFoodSearch()
  const USDA_KEY_AVAILABLE = !!import.meta.env.VITE_USDA_API_KEY
  ...
  const toggleFavourite = title => {
    setFavourites(prev => {
      ...
      const isFav   = next.has(title)
      if (isFav) next.delete(title); else next.add(title)
      localStorage.setItem("favouriteRecipes", JSON.stringify([...next]))
      // Find recipe id to sync
      const rec = readStorage("savedRecipes", []).find(r => r.title === title)
      if (rec) syncFavourite(rec.id, !isFav)
      return next
    })
  }
      ...
      <div className="absolute top-6 right-6 z-10 flex items-center gap-2">
        {user ? (
          ...
          <button onClick={() => setShowAuth(true)}
            className={`px-4 py-2 rounded-full text-sm font-medium backdrop-blur border transition-colors ${isDark ? "bg-orange-500/20 border-orange-500/30 text-orange-400 hover:bg-orange-500/30" : "bg-orange-50 border-orange-200 text-orange-600 hover:bg-orange-100 shadow-sm"}`}>
            ☁️ Sign In
          </button>
        )}
        ...
        <div className={`relative backdrop-blur-xl border rounded-3xl p-8 shadow-2xl ${isDark ? "bg-white/5 border-white/10" : "bg-white border-gray-200"}`}>
          ...
          <motion.div animate={inputError ? { x: [-10, 10, -8, 8, -4, 4, 0] } : { x: 0 }}
            ...
            {usdaSuggestions.length > 0 && (
              <div className={`mt-1 rounded-2xl border overflow-hidden shadow-xl
                ${isDark ? "bg-[#111827] border-white/10" : "bg-white border-gray-200"}`}>
                <p className="text-xs text-gray-600 px-4 pt-3 pb-1 font-semibold uppercase tracking-wider">
                  🔬 USDA verified — click to add
                </p>
                {usdaSuggestions.map((s, i) => (
                  <button key={i}
                    onClick={() => {
                      const name = s.name.split(",")[0].toLowerCase().trim()
                      setRawInput(prev => {
                    ...
                    <span className="text-xs text-gray-500 shrink-0 ml-2">
                      {s.nutrients?.cal ? `${Math.round(s.nutrients.cal)} kcal` : ""}
                      {s.nutrients?.p   ? ` · ${Math.round(s.nutrients.p)}g P` : ""}
                    </span>
                  </button>
                ))}
                <button onClick={() => setUsdaSuggestions([])}
                  className="w-full text-xs text-gray-700 py-2 hover:text-gray-500 transition-colors">
                  Dismiss
                </button>
              </div>
            )}
          ...
          <motion.div initial={{ opacity: 0, y: 60 }} animate={{ opacity: 1, y: 0 }}
            ...
            <RecipeErrorBoundary>
              <RecipeDisplay
                ...
                onSaveNote={(recipeId, note) => syncNote(recipeId, note)}
              />
            </RecipeErrorBoundary>
          </motion.div>
        )}

...
export function estimateQuantity(item, nutritionOverrides = {}) {
  const d = NUTRITION_DB[item] ?? nutritionOverrides[item]
  if (!d) return 80
...
export function generateSmartRecipe({
  ...
  // nutritionOverrides injects USDA real data for unknown ingredients.
  const totals = computeNutritionTotals(lower, nutritionOverrides)
